...
async def debug_professional_specialties(
    ...
    try:
        ...
        specialties_count = db.query(ProfessionalSpecialty).filter(
...
async def get_professional_specialties(
    ...
    try:
        ...
        specialties = db.query(ProfessionalSpecialty).filter(
            ProfessionalSpecialty.professional_id == user_uuid,
...
async def create_professional_specialty(
    ...
    if specialty_data.specialty_id:
        ...
        existing_assignment = db.query(ProfessionalSpecialty).filter(
    ...
    if specialty_data.is_default:
        ...
        db.query(ProfessionalSpecialty).filter(
            ProfessionalSpecialty.professional_id == current_user_id,
            ProfessionalSpecialty.is_active == True,
            ProfessionalSpecialty.is_default == True
    ...
    try:
        from uuid import UUID
        user_uuid = UUID(current_user_id)
        specialty_uuid = UUID(specialty_data.specialty_id) if specialty_data.specialty_id else None
    except ValueError as e:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
...
async def get_professional_specialty(
    ...
    specialty = db.query(ProfessionalSpecialty).filter(
...
async def get_available_default_specialties(
    ...
    added_specialty_ids = db.query(ProfessionalSpecialty.specialty_id).filter(
        ProfessionalSpecialty.professional_id == current_user_id,
        ProfessionalSpecialty.is_active == True,
