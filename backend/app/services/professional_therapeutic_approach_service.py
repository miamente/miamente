...
class ProfessionalTherapeuticApproachService:
    ...
    def get_professional_therapeutic_approach(self, approach_id: str) -> Optional[ProfessionalTherapeuticApproach]:
        ...
        return self.db.query(ProfessionalTherapeuticApproach).filter(ProfessionalTherapeuticApproach.id == approach_id).first()
    ...
    def get_professional_therapeutic_approaches(self, professional_id: str) -> List[ProfessionalTherapeuticApproach]:
        ...
        return self.db.query(ProfessionalTherapeuticApproach).filter(ProfessionalTherapeuticApproach.professional_id == professional_id).all()

    def create_professional_therapeutic_approach(self, approach: ProfessionalTherapeuticApproachCreate) -> ProfessionalTherapeuticApproach:
    ...
    def update_professional_therapeutic_approach(self, approach_id: str, approach_update: ProfessionalTherapeuticApproachUpdate) -> Optional[ProfessionalTherapeuticApproach]:
    ...
    def add_therapeutic_approaches_to_professional(self, professional_id: str, approach_ids: List[str]) -> List[ProfessionalTherapeuticApproach]:
        """Add multiple therapeutic approaches to a professional."""
        # Remove existing therapeutic approaches for this professional
        self.db.query(ProfessionalTherapeuticApproach).filter(ProfessionalTherapeuticApproach.professional_id == professional_id).delete()

        # Add new therapeutic approaches
        new_approaches = []
        for approach_id in approach_ids:
            approach = ProfessionalTherapeuticApproachCreate(professional_id=professional_id, therapeutic_approach_id=approach_id)
            db_approach = ProfessionalTherapeuticApproach(**approach.dict())
            self.db.add(db_approach)
