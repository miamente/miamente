 ...
 */
export function createJitsiUrl(appointmentId: string, baseUrl?: string): string {
 ...
 */
export function generateConfirmationSubject(appointmentDate: Date): string {
 ...
 * @returns Email subject
 */
export function generateReminderSubject(appointmentDate: Date, hoursUntil: number): string {
  const formattedDate = formatEmailDate(appointmentDate);
  if (hoursUntil === 24) {
