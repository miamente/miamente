 ...
 */
async function sendReminderEmail(appointment: any, hoursUntil: number): Promise<boolean> {
  try {
    // Get user details
 ...
 */
async function markReminderSent(appointmentId: string, reminderType: "24h" | "1h"): Promise<void> {
  try {
 ...
 */
async function getAppointmentsForReminders(): Promise<{
  "24h": any[];
  "1h": any[];
}> {
  const now = new Date();
  ...
  try {
    ...
    const appointments24h = appointments24hSnapshot.docs.map(
