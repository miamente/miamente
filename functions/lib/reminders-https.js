...
async function sendReminderEmail(appointment, hoursUntil) {
    try {
        ...
        // Generate email HTML
        const emailHtml = (0, email_1.generateReminderEmailHtml)(appointmentDate, jitsiUrl, hoursUntil, appointment.professional?.fullName);
        // Send email
        const result = await (0, email_1.sendEmailHandler)(userEmail, `Recordatorio: Cita en ${hoursUntil} hora${hoursUntil > 1 ? 's' : ''}`, emailHtml);
        if (result.success) {
            v2_1.logger.info(`Reminder email sent for appointment ${appointment.id} (${hoursUntil}h)`);
            return true;
        }
        else {
            v2_1.logger.error(`Failed to send reminder email for appointment ${appointment.id}:`, result.error);
            return false;
        }
    }
    catch (error) {
...
exports.runReminders = (0, https_1.onRequest)({ region: 'us-central1' }, async (req, res) => {
    ...
    try {
        ...
        v2_1.logger.info(`Sent ${result.remindersSent['24h']} 24h reminders and ${result.remindersSent['1h']} 1h reminders`);
