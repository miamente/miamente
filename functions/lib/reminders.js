...
import { generateConfirmationEmailHtml, generateReminderEmailHtml, generatePostSessionEmailHtml, } from "./email";
 ...
 */
export async function sendConfirmationEmail(appointmentId) {
    try {
        ...
        const html = generateConfirmationEmailHtml(appointmentData.start.toDate(), appointmentData.jitsiUrl, proData?.fullName);
        ...
        if (result.success) {
            ...
            logger.error(`Failed to send confirmation email for appointment ${appointmentId}:`, result.error);
        }
    }
    catch (error) {
...
export async function sendReminderEmails() {
    ...
    try {
        ...
        logger.info(`Sent ${appointments24h.docs.length} 24h reminders and ${appointments1h.docs.length} 1h reminders`);
    }
    catch (error) {
...
async function sendReminderEmail(appointmentId, appointment, hoursUntil) {
    try {
        ...
        const html = generateReminderEmailHtml(appointmentDate, appointmentData.jitsiUrl, hoursUntil, proData?.fullName);
        const result = await sendEmailHandler(userData.email, subject, html);
        if (result.success) {
...
async function sendPostSessionEmail(appointmentId, appointment) {
    try {
        ...
        if (result.success) {
            ...
            logger.error(`Failed to send post-session email for appointment ${appointmentId}:`, result.error);
        }
    }
    catch (error) {
