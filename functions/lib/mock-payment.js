...
exports.mockApprovePayment = (0, https_1.onCall)({ region: 'us-central1' }, async (request) => {
    ...
    if (!auth) {
        throw new https_1.HttpsError('unauthenticated', 'User must be authenticated to approve payments');
    }
    // Validate input
    if (!appointmentId) {
    ...
    try {
        ...
        const result = await db.runTransaction(async (transaction) => {
            ...
            if (appointmentData?.userId !== auth.uid) {
                throw new https_1.HttpsError('permission-denied', 'You can only approve your own appointments');
            }
            ...
            if (!['pending_payment', 'pending_confirmation'].includes(appointmentData?.status)) {
                throw new https_1.HttpsError('failed-precondition', `Appointment status '${appointmentData?.status}' cannot be approved`);
            }
            // Generate Jitsi URL
