 ...
 */
export const bookAppointment = onCall({ region: "us-central1" }, async (request) => {
    ...
    try {
        ...
        const result = await db.runTransaction(async (transaction) => {
            ...
            if (slotData?.professionalId !== proId) {
                throw new HttpsError("invalid-argument", "Slot does not belong to the specified professional");
            }
 ...
 */
export const getAppointment = onCall({ region: "us-central1" }, async (request) => {
 ...
 */
export const cancelAppointment = onCall({ region: "us-central1" }, async (request) => {
    const { appointmentId } = request.data;
    const { auth } = request;
    // Verify authentication
    if (!auth) {
