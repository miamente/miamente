...
class FirestoreBackup {
  ...
  async exportCollection(collectionName) {
    ...
    try {
      ...
      console.log(
        `✅ ${collectionName}: ${documents.length} documents, ${this.formatBytes(fileSize)}, ${duration}ms`,
      );
    } catch (error) {
  ...
  async uploadToStorage() {
    ...
    console.log(
      `☁️ Uploading backup to Firebase Storage: gs://${this.bucketName}/backups/${this.backupId}/`,
    );
