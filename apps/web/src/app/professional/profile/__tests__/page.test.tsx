...
vi.mock('@/components/professional-info/ProfessionalInfoForm', () => ({
  ProfessionalInfoForm: ({ professionalId, onSave }: { professionalId: string; onSave: (data: Record<string, unknown>) => void }) => (
    <div data-testid="professional-info-form">
      <div data-testid="professional-id">{professionalId}</div>
      <button 
        data-testid="save-button"
        onClick={() => onSave({ test: 'data' })}
      >
        Save
      </button>
    </div>
...
describe('ProfessionalProfilePage', () => {
  ...
  it('should render the page title and description', () => {
    ...
    expect(screen.getByText('Actualiza tu información profesional, especialidades, enfoques terapéuticos y modalidades de intervención.')).toBeInTheDocument();
  });
