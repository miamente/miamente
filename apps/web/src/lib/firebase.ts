...
export function getFirebaseApp(): FirebaseApp {
  ...
  const hasValidConfig =
    process.env.NEXT_PUBLIC_FB_API_KEY && process.env.NEXT_PUBLIC_FB_PROJECT_ID;
  ...
  if (typeof window !== "undefined") {
    ...
    if (isProduction) {
      ...
      console.log("📊 Analytics disabled for development mode");
    }
  }
  ...
  if (!auth) {
    ...
    if (isDevelopment && !emulatorsConnected.auth) {
      try {
        ...
        console.log(
          "⚠️ Auth emulator already connected or not available:",
  ...
  if (!firestore) {
    ...
    if (isDevelopment && !emulatorsConnected.firestore) {
      try {
        ...
        console.log(
          "⚠️ Firestore emulator already connected or not available:",
          error instanceof Error ? error.message : String(error),
        );
      }
    }
  ...
  if (!functions) {
    ...
    if (isDevelopment && !emulatorsConnected.functions) {
      try {
        ...
        console.log(
          "⚠️ Functions emulator already connected or not available:",
          error instanceof Error ? error.message : String(error),
        );
      }
    }
